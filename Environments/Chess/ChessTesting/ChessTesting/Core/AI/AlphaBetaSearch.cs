namespace ChessTesting
{
	...
	public class AlphaBetaSearch : ISearch {
		...
		public AlphaBetaSearch (Board board, AlphaBetaSettings settings) {
			this.board = board;
			...
			if (settings.useIterativeDeepening) {
				...
				for (int searchDepth = 1; searchDepth <= targetDepth; searchDepth++) {
					...
					if (abortSearch) {
						...
						// Update diagnostics
						Diagnostics.lastCompletedDepth = searchDepth;
						Diagnostics.move = bestMove.Name;
						Diagnostics.eval = bestEval;
						Diagnostics.moveVal = ChessTesting.PGNCreator.NotationFromMove (FenUtility.CurrentFen (board), bestMove);
		...
		int QuiescenceSearch (int alpha, int beta) {
			...
			Diagnostics.numPositionsEvaluated++;
			if (eval >= beta) {
				return beta;
			}
			if (eval > alpha) {
