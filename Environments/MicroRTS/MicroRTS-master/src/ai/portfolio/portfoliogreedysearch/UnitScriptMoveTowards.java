 ...
 * @author santi
 */
public class UnitScriptMoveTowards extends UnitScript {
    
    AbstractAction action;
    PathFinding pf;
    
    public UnitScriptMoveTowards(PathFinding a_pf) {
        pf = a_pf;
    }
    ...
    public UnitScript instantiate(Unit u, GameState gs) {
        ...
        if (bestEnemy != null) {
            UnitScriptMoveTowards script = new UnitScriptMoveTowards(pf);            
            script.action = new Move(u, Math.abs(bestEnemy.getX() - u.getX()) / 2, Math.abs(bestEnemy.getY() - u.getY()) / 2, pf);
            return script;
        } else {
            return null;
        }
