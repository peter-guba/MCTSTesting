...
{
    internal class WPExtraDataCruncher
    {
        /// <summary>
        ...
        public static void CrunchWPExtraData(string[] args)
        {
        ...
        /// <summary>
        /// Inserts dummy values in all the places where they need to be in order for the cruncher to work
        /// properly. That is, in all the 10000 vs 10000 playouts slots as well as in all the slots where
        /// neither of the competing algorithms is WP MCTS.
        /// </summary>
        private static void InsertDummyValues()
        {
            string[] battleSettings = new string[] { "4v4_2D-2B", "8v8_4D-4B", "16v16_8D-8B" };
            string[] algorithms = sheets[("4v4_2D-2B", 1000)].Keys.ToArray();
            ...
            foreach (string bs in battleSettings)
            {
                sheets.Add((bs, 10000), new Dictionary<string, Dictionary<string, (int, int, List<double>, List<double>, int, int)>>());
                foreach (string mcts1 in algorithms)
                {
                    sheets[(bs, 10000)].Add(mcts1, new Dictionary<string, (int, int, List<double>, List<double>, int, int)>());
                    foreach (string mcts2 in algorithms)
                    {
                        if (mcts1 == mcts2)
                        {
                            sheets[(bs, 10000)][mcts1].Add(mcts2, (0, 0, new List<double>(), new List<double>(), 0, 0));
                        }
                        else
                        {
                            sheets[(bs, 10000)][mcts1].Add(mcts2, (0, 0, new List<double>(dummyList), new List<double>(dummyList), 0, 0));
                        }

                        if (mcts1 != "WP" && mcts2 != "WP" && mcts1 != mcts2)
                        {
                            sheets[(bs, 1000)][mcts1].Add(mcts2, (0, 0, new List<double>(dummyList), new List<double>(dummyList), 0, 0));
                            sheets[(bs, 5000)][mcts1].Add(mcts2, (0, 0, new List<double>(dummyList), new List<double>(dummyList), 0, 0));
                        }
                    }
                }
            }
